# test_neuron_competition_v16.py
import random
import numpy as np
...
RUÍDO = 0.05
RECOMPENSA = 0.1
ESQUECIMENTO = 0.995   # decaimento mais lento
PENALIDADE_CARGA = 0.2 # penalidade mais forte
TEMPERATURA = 0.5      # softmax mais “suave”
...
class MultiEstado:
    ...
    def step(self, categoria):
        ...
        # Penalidade adaptativa não linear
        penalidade = PENALIDADE_CARGA * (self.uso_estado / max(1, np.sum(self.uso_estado)))**2
        
        # Score com afinidade, ruído, penalidade adaptativa
        score = self.afinidade[:, cat_idx] + np.random.random(NUM_ESTADOS) * RUÍDO - penalidade
